...
### 2026-07-13: Manuskript-Struktur — Impressum nach hinten, Quellenverzeichnis (pro Kapitel), Titel im Dateinamen
**Grund:** Anpassungen an PDF- und EPUB-Export (`manuskript/pdf/route.ts`, `lib/books/epub.ts`):
...
2. **Quellenverzeichnis** am Buchende, **nach Kapiteln gruppiert** und **nur tatsächlich verwendete** Quellen. Umsetzung: Das Kapitel-Modell hängt seine genutzten Quellen nach einem `===QUELLEN===`-Marker an (`prompts/kapitel.md` + `kapitel-vertiefen.md`); `splitChapterSources()` (`lib/books/sources.ts`) trennt den Block ab (nie im Kapiteltext sichtbar) und speichert ihn strukturiert in `chapters.sources` (jsonb, neue Migration `20260713120000_chapters_add_sources.sql`). Der Export lädt `chapters.sources` **best-effort** (fehlt die Migration, wird ohne Quellen exportiert statt zu brechen) und rendert je Kapitel mit Quellen eine Gruppe. Kein Kapitel-Quellen → kein Abschnitt. Der frühere Ansatz (alle URLs aus `projects.research` scrapen) ist damit ersetzt; `extractSources` entfernt.
3. **Dateiname = Buchtitel** statt Projekt-UUID: `manuscriptDisposition()` (`lib/books/filename.ts`) liefert ASCII-Slug als `filename` + RFC-5987 `filename*` mit UTF-8-Titel (Umlaute bleiben).
